...
public class LLRealtimeSegmentDataManager extends SegmentDataManager {
  ...
  private final Map<String, String> _partitioners;
  ...
  public LLRealtimeSegmentDataManager(RealtimeSegmentZKMetadata segmentZKMetadata, AbstractTableConfig tableConfig,
    ...
    // Partitioners
    _partitioners = indexingConfig.getPartitioners();
    if (_partitioners != null && _partitioners.size() > 0) {
      try {
        ...
        segmentLogger.info("Couldn't get number of partitions in 5s, not using partition config");
      }
    }
  ...
  private SegmentPartitionConfig createPartitionConfig() {
    ...
    for (String column : _partitioners.keySet()) {
      map.put(column, new ColumnPartitionConfig(
          _partitioners.get(column) + ColumnPartitionConfig.PARTITIONER_DELIMITER + _nPartitions,
